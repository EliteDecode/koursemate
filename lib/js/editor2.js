        ...
        insert_link: {
          ...
          beforeLoad: function () {
            ...
            if ($(editorObj).data("currentRange") != "") {
              $("#inputText_" + _idSuffix).val(
                $(editorObj).data("currentRange")
              );
            }
          },
          onSave: function () {
            ...
            if (
              ...
              navigator.userAgent.match(/Windows NT.*Trident\//)
            ) {
              var targetLink =
                '<a href="' +
                targetURL +
          ...
          onSave: function () {
            ...
            if ($("#imageList" + _idSuffix).data("current")) {
              if (
                ...
                var imageStr =
                  '<img src="' +
                  $("#imageList" + _idSuffix).data("current") +
                  '"/>';
      ...
      var modalTrigger = $("<a/>", {
        ...
        (function (e) {
          return function () {
            var _idSuffix = "_" + editorObj.attr("id") + "_Edt";
            $("#context-menu").remove();
            $("#tblRows" + _idSuffix).val(
              $(e.target).closest("table").prop("rows").length
            );
            $("#tblColumns" + _idSuffix).val(
            ...
            $("#tblWidth" + _idSuffix).val(
              $(e.target).closest("table").get(0).style.width
            );
            $("#tblHeight" + _idSuffix).val(
              $(e.target).closest("table").get(0).style.height
            );
            $("#tblAlign" + _idSuffix).val(
              $(e.target).closest("table").attr("align")
            );
            $("#tblBorder" + _idSuffix).val(
              $(e.target).closest("table").attr("border")
            );
            $("#tblCellspacing" + _idSuffix).val(
              $(e.target).closest("table").attr("cellspacing")
            );
            $("#tblCellpadding" + _idSuffix).val(
